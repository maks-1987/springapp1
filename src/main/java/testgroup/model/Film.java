...
public class Film {
     ...
     * класса в БД. @Entity — указывает на то, что данный класс является сущностью. @Table — указывает
     * на конкретную таблицу для отображения этой сущности. @Id — указывает, что данное поле
     * является первичным ключом, т.е. это свойство будет использоваться для идентификации каждой
     * уникальной записи. @Column — связывает поле со столбцом таблицы. Если имена поля и столбца
     * таблицы совпадают, можно не указывать. @GeneratedValue — свойство будет генерироваться
     * автоматически, в скобках можно указать каким образом. Достаточно знать, что в данном случае
     * каждое новое значение будет увеличиваться на 1 от предыдущего. В документации Hibernate
     * применять аннотации рекомендуется не к полям, а к геттерам
     */
    @Id
    @Column(name = "id")
